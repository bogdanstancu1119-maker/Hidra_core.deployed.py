...
class HydraEmailOrchestrator:
    ...
    def ruleaza_autonom(self):
        ...
        passimport os, re, json, pathlib, time
...
class HydraEmailOrchestrator:
    def __init__(self):
        ...
        self.roiu.mkdir(exist_ok=True)
        self.arhiva = self.roiu / "email_archive.jsonl"
        self.checkpoint = self.roiu / "email_checkpoint.txt"
        self.confirmari = self.roiu / "confirmari_auto.jsonl"
        
        # Checkpoint - de unde a rămas, nu de la 0
        self.deja_procesate = set()
        if self.arhiva.exists():
            try:
                for line in self.arhiva.read_text(encoding="utf-8").splitlines()[-500:]:
                    self.deja_procesate.add(json.loads(line).get("h",""))
            except: pass

    def ruleaza_autonom(self, emails_raw):
        # IA MAX 20 PE RUN, NU 2000! Asta rezolva 7/20
        lot = emails_raw[:20]  # frana PSIE
        print(f"[HYDRA FRANA] Procesam doar 20 din {len(emails_raw)} - restul in heartbeat-ul urmator")
        
        procesate_acum = 0
        for mail in lot:
            h = str(hash(mail.get("subject","")+mail.get("from","")))[:12]
            if h in self.deja_procesate:
                continue
            ...
            try:
                subj = str(mail.get("subject","")).lower()
                if "verif" in subj or "code" in subj or "otp" in subj:
                    # extrage cod
                    body = str(mail.get("body",""))
                    cod = re.search(r'\b(\d{6})\b', body)
                    if cod:
                        confirm = {"t": str(datetime.now()), "cod": cod.group(1), "de_la": mail.get("from"), "h": h}
                        with open(self.confirmari, "a", encoding="utf-8") as f:
                            f.write(json.dumps(confirm, ensure_ascii=False)+"\n")
                        print(f"[CONFIRMATA SINGURA] {cod.group(1)} de la {mail.get('from')}")
                
                with open(self.arhiva, "a", encoding="utf-8") as f:
                    f.write(json.dumps(log, ensure_ascii=False)+"\n")
                procesate_acum += 1
                time.sleep(0.5) # nu omori API-ul Gmail
            except Exception as e:
                print(f"[EROARE EMAIL] {e} - continui")
                continue
